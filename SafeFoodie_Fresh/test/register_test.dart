...
void main() {
  group('register page: ', () {
    //Test 1
    testWidgets('email field should validate only when criteria is met',
        (WidgetTester testEmail) async {
      ...
      final EditableTextState emailFieldTrue =
          testEmail.state<EditableTextState>(find.byType(EditableText));
      emailFieldTrue.updateEditingValue(const TextEditingValue(
          text: 'spappTest@gmail.com', composing: TextRange(start: 2, end: 5)));
      expect(emailFieldTrue.currentTextEditingValue.composing,
      ...
      final EditableTextState emailFieldFalse =
          testEmail.state<EditableTextState>(find.byType(EditableText));
      emailFieldFalse.updateEditingValue(const TextEditingValue(
          text: 'spappTest', composing: TextRange(start: 2, end: 5)));
      expect(emailFieldFalse.currentTextEditingValue.composing,
          const TextRange(start: 2, end: 5));
    ...
    //Test 2
    testWidgets('password field should validate only when criteria is met',
        (WidgetTester testPW) async {
      final GlobalKey<FormState> formKey = GlobalKey<FormState>();
      String? fieldValue;
      ...
      final EditableTextState emailFieldTrue =
          testPW.state<EditableTextState>(find.byType(EditableText));
      emailFieldTrue
          .updateEditingValue(const TextEditingValue(text: 'Test12345'));
...
//Test 3
    testWidgets(
        'when go back button is clicked, user should be sent back to home page',
        (WidgetTester testNav) async {
      var goBack = false;
    ...
    //Test 5
    testWidgets('validates even before text boxes have input',
    ...
    //Test 6
    testWidgets('password field will be obscured',
        (WidgetTester testObscure) async {
